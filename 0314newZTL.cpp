...
void* periodicThread(void* arg) {
    ...
    timespec_add_ns(&now, NSEC_PER_MSEC);  // Add 1 ms to the current time,first trigger after 1ms
    value.it_value = now;
    ...
    for (size_t i = 0; i < CYCLES; i++) {
        ...
        //long long elapsed_ns = (after.tv_sec - before.tv_sec) * NSEC_PER_SEC + (after.tv_nsec - before.tv_nsec);
        //printf("good - Computation took %lld ns\n", elapsed_ns);
    }
